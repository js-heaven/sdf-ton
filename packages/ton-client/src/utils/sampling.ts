...
export default function startSampling(
  ...
  gl.texImage2D(
    ...
    0,
    gl.RGBA32F,
    options.sqrtBufferSize / 4,
    options.sqrtBufferSize,
    0,
    gl.RGBA,
    gl.FLOAT,
    null
  ...
  const samplePass = () => {
    ...
    planeEndAngle =
      bufferDuration * planeFrequency * Math.PI * 2 + planeStartAngle;
    ...
    audioContext.audioWorklet.addModule('./worklet.js').then(() => {
      ...
      continousBufferNode.port.onmessage = (event) => {
        if (event.data.type == 'requestBuffer') {
          ...
          const assumedCurrentBuffer =
            generatedBufferCounter - options.numberOfBuffers;
  ...
  return {
    ...
    getPeriodBeginAndLength: () => [
      periodStartSample - bufferStartSample,
      periodLength,
    ],
