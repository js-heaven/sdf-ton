...
import {
  compileShaders,
  makeUniformLocationAccessor,
} from './utils/shader-tools';
...
window.addEventListener('load', () => {
  ...
  const renderPass = () => {
    ...
    if (sampleTex) {
      [swipeA, swipeB] = getPlaneSegment().map((angle) => [
        Math.cos(angle),
        Math.sin(angle),
      ]);
    }
  ...
  const visualizePass = () => {
    ...
    gl.uniform1f(
      visualizeUniLocs.normalizeFactor,
      normalizeInfo.normalizeFactor
    );
